...
async def command_start_handler(
    ...
    await message.answer(
        text=_("Рады видеть тебя в нашем ITAM bot!\n\n Настоятельно просим тебя перейти в меню ⚙️ Профиль и заполнить свои данные, это позволит тебе получить доступ ко всем нашим крутым функциям (PS мы готовим большое обновление, которе позволит тебе получать плюшки за активность!)\n\n Также зайди в раздел 🏢 Клубы и подпишись на клубы, которые тебе интересны!"),
        reply_markup=base_menu_reply_key(),
    )
    await asyncio.sleep(0.5)
    await message.answer(
